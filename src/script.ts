...
interface powers_property {
    name:string,
    flag : boolean,
    description: string,
    element_choice ?: number
}


const Powers: powers_property[]= [
    {
        name:'Element_Boost',
        flag: false,
...
const pChoice:HTMLElement = <HTMLElement>document.getElementById("playerChoice");
const cChoice:HTMLElement = <HTMLElement>document.getElementById("computerChoice");
const pscore:HTMLElement = <HTMLElement>document.getElementById('PlayerScore');
const cscore:HTMLElement = <HTMLElement>document.getElementById('ComputerScore');
var rounds:HTMLCollectionOf<HTMLElement> = <HTMLCollectionOf<HTMLElement>>document.getElementsByClassName("rounds");

const powerupchance:number = Math.floor(Math.random() * (6 - 2) + 2);
var computerchoice:number; 

function noOfRounds():void {
    // Get the input element by its id
    var numberInput:HTMLInputElement = <HTMLInputElement>document.getElementById("no_of_rounds");
    // Get the value entered by the user
    
    let a:number =parseInt(numberInput.value);
    console.log(a);
...
function enable_buttons(pv:string):void {
    const element = document.getElementById(pv);
    element?.removeAttribute("disabled");
}


function choice(ii:string):void {
    let i = parseInt(ii);
    // console.log(Powers);
...
function updateround(i:number):void {
    if (i !== computerchoice) {

        // rounds = document.getElementsByClassName("rounds");
        var num:number = Number(rounds[1].innerText) + 1;
...
function powerup(num:number):void {
...
function pop_alert(txt:string):void {
...
function winner_declaration(rounds:HTMLCollectionOf<HTMLElement>):void {
...
function addpoint(element: HTMLElement) {
    let num:number = Number(element.innerText) + 1;
    element.innerText = String(num);
}

function subractpoint(element: HTMLElement) {
    let num:number = Number(element.innerText) - 1;
    element.innerText = String(num);
}
...
function Element_Boost(i:string) {
...
function Element_Swap(ii:string) {
    let i = parseInt(ii);
